...
Graph* GraphCreateTranspose(const Graph* g) {
    assert(g != NULL);                // Ensure the graph isn't NULL
    assert(g->isDigraph);             // Ensure it's a directed graph
    assert(!g->isComplete);           // Ensure it's not a complete graph

    // Create the transposed graph with the same number of vertices and properties
    Graph* transposed = GraphCreate(g->numVertices, g->isDigraph, g->isWeighted);

    // Iterate over all vertices of the original graph
    List* vertices = g->verticesList;
    ListMoveToHead(vertices);
    for (unsigned int u = 0; u < g->numVertices; ListMoveToNext(vertices), u++) {
        struct _Vertex* v = ListGetCurrentItem(vertices);

        // Get the adjacency list for vertex u
        List* adjList = v->edgesList;
        ListMoveToHead(adjList);

        // For each adjacent vertex v of u, insert the transposed edge v -> u
        while (ListCurrentIsInside(adjList)) {
            struct _Edge* e = ListGetCurrentItem(adjList);
            unsigned int adjVertex = e->adjVertex;

            // Insert the transposed edge adjVertex -> u
            GraphAddEdge(transposed, adjVertex, u);
